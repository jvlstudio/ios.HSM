...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"Puxe para atualizar",),
                             NSLocalizedString(@"Solte para atualizar",),
                             NSLocalizedString(@"Atualizando, aguarde...",),
                                nil];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        titleAttr       = [[NSAttributedString alloc] initWithString:self.titleLabel.text
                                                          attributes:@{ NSFontAttributeName: self.titleLabel.font}];
        CGRect rectTit  = [titleAttr boundingRectWithSize:(CGSize){labelMaxWidth, self.titleLabel.font.lineHeight}
                                                  options:NSStringDrawingUsesLineFragmentOrigin
                                                  context:nil];
        CGSize titleSize    = rectTit.size;
        
        // size > subtitle
        NSAttributedString *subTitleAttr;
        if(!self.subtitleLabel.text)
            self.subtitleLabel.text = @"";
        subTitleAttr    = [[NSAttributedString alloc] initWithString:self.subtitleLabel.text
                                                          attributes:@{ NSFontAttributeName: self.subtitleLabel.font}];
        CGRect rectSub  = [subTitleAttr boundingRectWithSize:(CGSize){labelMaxWidth, self.subtitleLabel.font.lineHeight}
                                                     options:NSStringDrawingUsesLineFragmentOrigin
                                                     context:nil];
        CGSize subtitleSize = rectSub.size;
